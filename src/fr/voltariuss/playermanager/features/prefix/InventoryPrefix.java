...
public final class InventoryPrefix {
	...
	private static final List<String> LORE_ACTIVATION_INFO = Arrays.asList("",
			ChatColor.GRAY + UtilsAPI.INVENTORY_INDICATION_TO_ACTIVATE);
	...
	public static HashMap<Integer, ItemInteractive> getInventoryItemMap(OfflinePlayer target) throws SQLException {
		...
		for (SubRank subRank : SubRank.values()) {
			...
			if (subRank != SubRank.VIP && subRank != SubRank.VIP_PLUS) {
				...
				ItemInteractive item = new ItemInteractive(ItemUtils.generateItem(Material.INK_SACK, 1, (short) 8,
						getPrefixItemName(subRank.getPrefix().toString(), UtilsPlayerManager.PREFIX_UNAVAILABLE_TAG),
						LORE_ACTIVATION_INFO));
				...
				if (subRanks.contains(subRank)) {
					...
					meta.setDisplayName(meta.getDisplayName().replace(UtilsPlayerManager.PREFIX_UNAVAILABLE_TAG,
							UtilsPlayerManager.PREFIX_AVAILABLE_TAG));
					item.setItemMeta(meta);

					if (prefixType.equalsIgnoreCase(subRank.name())) {
						// S'il s'agit du préfixe utilisé par le joueur, alors on lui ajoute une
						// disctinction
					...
					ItemMeta meta = item.getItemMeta();
					meta.setLore(
							Arrays.asList("", UtilsPlayerManager.PREFIX_SUBRANK_TERM_TAG + subRank.getColoredName()));
					item.setItemMeta(meta);
		...
		if (subRanks.contains(SubRank.VIP_PLUS)) {
			...
			meta.setDisplayName(getPrefixItemName(SubRank.VIP_PLUS.getPrefix().toString(),
					UtilsPlayerManager.PREFIX_AVAILABLE_TAG));
			...
			ItemMeta meta = vipPrefixItem.getItemMeta();
			meta.setDisplayName(
					getPrefixItemName(SubRank.VIP.getPrefix().toString(), UtilsPlayerManager.PREFIX_AVAILABLE_TAG));
			vipPrefixItem.setItemMeta(meta);
		}
